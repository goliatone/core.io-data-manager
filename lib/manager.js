...
class Manager extends EventEmitter {
    ...
    import(type, content, options={}){
        ...
        return Promise.resolve(this._parsers[type](content, options)).then((results) => {
            ...
            this.logger.error('import error for type %s', type);
            return err;
        });
    }
    ...
    importAsModels(identity, type, content, options={}){
        return this.import(type, content, options).then((results)=>{
            ...
            this.logger.error('importAsModels error for identity "%s", type "%s"', identity, type);
            return err;
        });
    }
    ...
    importFileAsModels(identity, filename, options){
        return this.importFile(filename, options).then((results)=>{
            ...
            this.logger.error('importFileAsModels error importing file %s', filename);
            return err;
        });
    }
    ...
    _importModel(identity, items, options={}){
        ...
        return this.modelProvider(identity).then((Model)=>{
            ...
            let method = options.truncate ? 'create' : 'updateOrCreate';
            ...
            function iterate(records, options, output=[], errors=[]){
                ...
                let args = options.truncate ? [record] : [criteria, record];

                return Model[method].apply(Model, args).then((record) => {
                    output.push(record);
                    return iterate(records, options, output, errors);
                }).catch((err)=>{
                    _logger.error('_importModel.iterate error');
                    _logger.error('%s.%s failed: %s', identity, method, err.message);
                    _logger.error('DataManager does not know what to do with this error.');
                    _logger.error('They will be bubbled up, you should handle them.');
                    errors.push(err);
                    return iterate(records, options, output, errors);
                });
            }
            ...
            this.logger.error('_importModel error, modelProvider failed');
            return err;
        });
    }
    ...
    exportModels(identity, query={}, type='json', options={}){
        ...
        return this.modelProvider(identity).then((Model)=>{
            ...
            this.logger.error('exportModels error for identity "%s", type "%s"', identity, type);
            return err;
        });
    }
    ...
    exportModelsToFile(identity, query={}, type='json', options={}){
        ...
        return this.exportModels(identity, query, type, options).then((output)=>{
            ...
            this.logger.error('exportModelsToFile error for identity "%s", type "%s"', identity, type);
            return err;
        });
    }
}
