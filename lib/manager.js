...
class Manager extends EventEmitter {
    ...
    /**
     * Import a file a parse it's contents.
     * By default we use the file extension to find a matching
     * parser. If you want to set the parser you can set
     * `options.type` to the desired value.
     * 
     * This is a convenience method that just loads a file
     * and calls `DataManager.import`.
     * 
     * @param {String} filename Path to file to be imported
        ...
        return this.modelProvider(identity).then(Model => {
            ...
            function iterate(records, options, output = [], errors = []) {
                ...
                return Model[updateStrategy].apply(Model, args).then(record => {
                    ...
                    if (this.debug) _logger.error(err);
